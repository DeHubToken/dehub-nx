  ...
  return (
    <>
      ...
      >
        <div>
          {
          CHAINS.filter((chain) => isSourceChain ? true : chain.chainID !== sourceChain?.chainID).map((chain: ChainType) => {
            return (
              <div
              ...
              >
                <div className="card gray shadow-2 mt-1 flex flex-row mt-3">
                  <img width={30} height={30} className="align-left" src={chain.logo}></img>
                  <div className="text right align-self-center" style={{fontSize:'15px', paddingLeft:'15px'}}>{chain.name}</div>
                </div>
              </div>
            );
          })}
