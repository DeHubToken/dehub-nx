...
import { getBep20Contract, getBridgeContract, getDehubTokenContract, getStakingContract } from '../utils/contractHelpers';
...
export const useDehubTokenContract = (): Contract | null => {
  ...
  return useMemo(
    () =>
      web3 && chainId 
        ? getDehubTokenContract(chainId, web3.getSigner())
        : null,
    [web3, chainId]
  );
}
...
export const useBridgeContract = (): Contract | null => {
  ...
  return useMemo(
    () =>
      web3 && chainId
        ? getBridgeContract(chainId, web3.getSigner())
        : null,
    [web3, chainId]
  );
}
